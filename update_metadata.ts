...
for (const file of files) {
  ...
  // Find all @Tool({ ... }) and append metadata: { category: '...' }
  content = content.replace(/@Tool\(\{\s*(.*?)\s*\}\)/gs, (match, options) => {
    if (options.includes('metadata:')) {
      return match;
    }
    return `@Tool({ ${options}, metadata: { category: '${mcpId}' } })`;
  });
  
  fs.writeFileSync(p, content);
  console.log(`Updated ${file} with category: ${mcpId}`);
}
